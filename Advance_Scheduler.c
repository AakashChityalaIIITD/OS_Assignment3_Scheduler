...
void sortTerminatedQueueByPriority(struct TerminatedQueue* queue) {
    for (int i = 0; i <= queue->rear; i++) {
        for (int j = i + 1; j <= queue->rear; j++) {
            if (queue->processes[i].priority < queue->processes[j].priority) {
                struct Process temp = queue->processes[i];
...
void printTerminatedQueue(struct TerminatedQueue* queue) {
    sortTerminatedQueueByPriority(queue);
    for (int i = 0; i <= queue->rear; i++) {
        printf("Terminated %s Process with PID %d:\n",queue->processes[i].command ,queue->processes[i].pid);
    ...
    if (scheduler_pid == 0) {
        ...
        while (1) {
            ...
            if (i > scheduler_queue->rear || i >= MAX_PROCESSES) {
                i = 0;
                sortQueueByPriority(scheduler_queue); // sort by priority at the start
            }
        ...
        char priority_str[MAX_PROGRAM_NAME];
        int priority = 1;
        
        // Read program name
        scanf("%s", program);
        
        // Try to read priority (optional)
        if (scanf("%s", priority_str) == 1 && priority_str[0] != '\0') {
            int temp_priority = atoi(priority_str);
            if (temp_priority > 0) {
            priority = temp_priority;
        } else {
            printf("Invalid priority value. Using default priority 1.\n");
            priority = 1;
            }
        }
        ...
        if (child_pid == 0) {
            ...
            printf("Submitted process %s with priority %d\n", program, priority);
